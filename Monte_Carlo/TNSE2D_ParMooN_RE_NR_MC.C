...
// =======================================================================
#include "../Examples/Monte_Carlo/Flow_Cylinder.h" //   in unit square
...
int main(int argc, char *argv[])
{
	...
	//=========================================================================
	std::ofstream parmoonOut;
	parmoonOut.open("pyReadIn.txt");
	...
	for (int Realisation = 0; Realisation < N_Realizations; Realisation++)
	{
		double RE_NR = rand()%300+ 10;
		TDatabase::ParamDB->RE_NR = RE_NR;
		cout << " ------------------------------------- REALISATION ----------------------------- " << endl;
		cout << " ----------------------"
			 << " NO : " << Realisation << "  RE_NR : " << TDatabase::ParamDB->RE_NR << " ---------------------- " << endl;
		...
		reNR.open("ReNo.txt");
		reNR << TDatabase::ParamDB->RE_NR << endl;
		...
		double normo = 100;
		double *old_soll = new double[N_TotalDOF];
		double old_norm = 0.0;
		double new_norm = 0.;
		// time loop starts
		while (TDatabase::TimeDB->CURRENTTIME< end_time)
		{
			cout << " NORM DIFFF : " << normo << endl;
			old_norm = new_norm; // time cycle
			m++;
...
exit(0);      */
			...
			for (int i = 0; i < N_TotalDOF; i++)
			{
				normo += pow((sol[i]  - old_soll[i]),2);
			}
			normo = sqrt(normo);
			memcpy(old_soll,sol, SizeOfDouble*N_TotalDOF);
