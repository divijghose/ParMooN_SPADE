...
void TriaReMeshGen(TDomain *&Domain)
{
  ...
  {
    if (left > In.pointlist[2 * i])
      left = In.pointlist[2 * i];
    if (right < In.pointlist[2 * i])
      right = In.pointlist[2 * i];
    if (top < In.pointlist[2 * i + 1])
      top = In.pointlist[2 * i + 1];
    if (bottom > In.pointlist[2 * i + 1])
      bottom = In.pointlist[2 * i + 1];
  }
  ...
  for (i = 0; i < N_G; i++)
    if (PointNeighb[i] > maxEpV)
      maxEpV = PointNeighb[i];
  ...
  for (i = 0; i < N_G; i++)
  {
    ...
    if (Out.edgemarkerlist[i]) // 0 for inner edges and Boundcomp+1 for Boundedge respect
    {
      ...
      else if (Domain->GetBdPart(0)->GetBdComp(CurrComp)->IsFreeBoundary())
    ...
    for (j = 0; j < 3; j++)
      if (Triangles[3 * Neib[0] + j] == a)
        break;

    // find the local index for the point 'b' on the cell
    for (k = 0; k < 3; k++)
      if (Triangles[3 * Neib[0] + k] == b)
        break;
    k = k * 10 + j;
    ...
    if (Neib[1] != -1)
    {
      // find the local index for the point 'a' on the cell
      for (j = 0; j < 3; j++)
        if (Triangles[3 * Neib[1] + j] == a)
          break;

      // find the local index for the point 'b' on the cell
      for (k = 0; k < 3; k++)
        if (Triangles[3 * Neib[1] + k] == b)
          break;
