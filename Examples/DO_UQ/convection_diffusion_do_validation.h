    ...
    { // new a loop
        ...
        { // Cell Loop
            ...
            for (int quadPt = 0; quadPt < N_Points2; quadPt++)
            {
                ...
                C_x_i[quadPt] = 0.0;
                C_y_i[quadPt] = 0.0;
                C_xx_i[N_Points2] = 0.0;
                C_yy_i[N_Points2] = 0.0;
            }

            for (int quadPt = 0; quadPt < N_Points2; quadPt++)
            {
                for (int bf = 0; bf < N_BaseFunct; bf++)
                {
                    int globDOF = DOF[bf];
                    C_i[quadPt] += origvaluesD00[quadPt][bf] * C_Array_i[globDOF];
                    C_x_i[quadPt] += origvaluesD10[quadPt][bf] * C_Array_i[globDOF];
                    C_y_i[quadPt] += origvaluesD01[quadPt][bf] * C_Array_i[globDOF];
                    C_xx_i[N_Points2] = origvaluesD20[quadPt][bf] * C_Array_i[globDOF];
                    C_yy_i[N_Points2] = origvaluesD02[quadPt][bf] * C_Array_i[globDOF];
                }
            }
            ...
            for (int quadPt = 0; quadPt < N_Points2; quadPt++)
            {
                ...
                ipval += ((-1.0 * b1 * C_x_i[quadPt]) + (-1.0 * b2 * C_y_i[quadPt]) + (eps * (C_xx_i[quadPt] + C_yy_i[quadPt]))) * C_a[quadPt] * Mult;
                // This is the inner product over the whole domain
            }
            for (int i = 0; i < MaxN_QuadPoints_2D; i++)
            {
                delete[] Coeffs[i];
        ...
        { // 2nd Cell Loop
            ...
            for (int quadPt = 0; quadPt < N_Points2; quadPt++)
            {
                ...
                for (int j = 0; j < N_BaseFunct; j++)
                {
                    rhs[j] += (-1.0 * ipval * C_a[quadPt]) * Mult * orgD00[j];
                }
            }
...
{ // function start
    ...
    memcpy(C_Array_i, C_Array + (i_index * lenMode), lenMode * SizeOfDouble);
    ...
    { // new a loop
        ...
        { // cell loop
            ...
            for (int quadPt = 0; quadPt < N_Points2; quadPt++)
            {
                ...
                ipval += ((-1.0 * b1 * C_x_a[quadPt]) + (-1.0 * b2 * C_y_a[quadPt]) + (eps * (C_xx_a[quadPt] + C_yy_a[quadPt]))) * C_i[quadPt] * Mult;
                // val *= Mult; //
            }
            for (int i = 0; i < MaxN_QuadPoints_2D; i++)
            {
