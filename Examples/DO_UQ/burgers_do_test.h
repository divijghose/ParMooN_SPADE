 ...
 */
void CalcCovarianceMatx(double* Vector, int height, int width){

    TDatabase::ParamDB->COVARIANCE_MATRIX_DO = new double[width* width]();

    cblas_dgemm(CblasRowMajor,CblasTrans,CblasNoTrans,width,width, height, (1.0/(height-1)), Vector,height,Vector,width,0.0,TDatabase::ParamDB->COVARIANCE_MATRIX_DO,width);
 ...
 * @remark TDatabase::ParamDB->COVARIANCE_MATRIX_DO is stored in Row Major form
 */
void CalcCoskewnessMatx(double* Vector, int height, int width){
        ...
        for(int j = 0; j < width; j++){
            for(int k = 0; k < width; k++){
                for(int p = 0; p < height; p++){
                    ...
                    TDatabase::ParamDB->COSKEWNESS_MATRIX_DO[width*width*k + width*i + j] += ((Vector[width*p+i]*Vector[width*p+i]*Vector[width*p+i])/(height-1));
...
void DO_Mean_RHS(TFESpace2D *Fespace, TFEVectFunct2D *FeVector_C, int N_S,double* GlobalRhs_mode , int i_index)
{
...
void DO_Mode_RHS(TFESpace2D *Fespace, TFEVectFunct2D *FeVector_C, int N_S,double* GlobalRhs_mode , int i_index)
{
  ...
  int* GlobalNumbers = Fespace->GetGlobalNumbers();
    ...
    int* DOF = GlobalNumbers + BeginIndex[cellId];
		...
		double* U_Mode = FeVector_C->GetValues();
		int len         = FeVector_C->GetLength();

		

		double* U_i = U_Mode + i_index*len;
		...
		for ( int a = 0 ; a < N_S ; a++)
		{
			double* C_Array_a = U_Mode + i_index*len;
		...
		for ( int j = 0 ; j < N_BaseFunct ; j++)
		{
			int GlobalDOF   				= DOF[j];
			GlobalRhs_mode[GlobalDOF]   	+= rhs[j];
      GlobalRhs_mode[GlobalDOF+lenMode]
		}
		// -- 
	}
}
